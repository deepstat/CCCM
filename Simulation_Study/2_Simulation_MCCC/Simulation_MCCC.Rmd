...
for(i in 1:nrow(cand_df)){
    ...
    if(dist == "multivariate_norm"){
        # True MCCC
        V_D  <- (U[1,1] + U[2,2] - U[1,2] - U[2,1]) * V +
            ((M[1,] - M[2,]) %*% t(M[1,] - M[2,]))
        V_I  <- (U[1,1] + U[2,2]) * V +
            ((M[1,] - M[2,]) %*% t(M[1,] - M[2,]))
        
        eigen_V_I    <- eigen(V_I)
        half_inv_V_I <- eigen_V_I$vectors %*%
            diag((eigen_V_I$values)^-.5) %*%
            t(eigen_V_I$vectors)
        
        MCCC <- 1 - sqrt(sum((half_inv_V_I %*% V_D %*% half_inv_V_I)^2)/5)
